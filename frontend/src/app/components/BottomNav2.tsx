"use client";

import React, { useState, useRef, useEffect } from "react";
import Link from "next/link";
import { usePathname } from "next/navigation";
import { navItems, allPages } from "@/config/nav";
import { ChevronsUpDown } from "lucide-react";

const BottomNav2 = () => {
    const pathname = usePathname();
    const [menuOpen, setMenuOpen] = useState(false);
    const dockRef = useRef<HTMLDivElement | null>(null);
    const iconsRef = useRef<Array<HTMLAnchorElement | null>>([]);
    const listRefs = useRef<Array<HTMLAnchorElement | null>>([]);
    const menuRef = useRef<HTMLDivElement | null>(null);

    // Two separate pills to avoid morphing transitions
    const [collapsedPillStyle, setCollapsedPillStyle] = useState<React.CSSProperties>({});
    const [expandedPillStyle, setExpandedPillStyle] = useState<React.CSSProperties>({});

    // Route checking logic from original BottomNav
    const isActiveRoute = (href: string) => {
        if (pathname === href) return true;
        if (pathname.startsWith(href + "/")) return true;
        if (
            href === "/app/dashboard" &&
            (pathname === "/app/dashboard" || pathname === "/app/app/dashboard")
        )
            return true;
        return false;
    };

    // Handle clicks outside to close menu
    useEffect(() => {
        const handleClick = (e: MouseEvent) => {
            const target = e.target as Node;
            if (menuOpen && dockRef.current && !dockRef.current.contains(target)) {
                setMenuOpen(false);
            }
        };

        document.addEventListener("mousedown", handleClick);
        return () => document.removeEventListener("mousedown", handleClick);
    }, [menuOpen]);

    // Position the moving glass pills
    useEffect(() => {
        const updatePills = () => {
            const dock = dockRef.current;
            if (!dock) return;

            // 1. Calculate Collapsed Pill (Bottom Row)
            // ONLY visible if menu is closed (or fading out)
            if (menuOpen) {
                setCollapsedPillStyle(prev => ({ ...prev, opacity: 0 }));
            } else {
                const activeIndex = navItems.findIndex((item) => isActiveRoute(item.href));
                if (activeIndex >= 0) {
                    const target = iconsRef.current[activeIndex];
                    if (target) {
                        const dockRect = dock.getBoundingClientRect();
                        const targetRect = target.getBoundingClientRect();
                        setCollapsedPillStyle({
                            transform: `translateX(${targetRect.left - dockRect.left}px)`,
                            bottom: `${dockRect.bottom - targetRect.bottom}px`,
                            width: `${targetRect.width}px`,
                            height: `${targetRect.height}px`,
                            opacity: 1,
                        });
                    }
                } else {
                    setCollapsedPillStyle(prev => ({ ...prev, opacity: 0 }));
                }
            }

            // 2. Calculate Expanded Pill (Vertical List)
            // Always update position if menuRef exists, visibility handled by container opacity
            const menuContainer = menuRef.current;
            if (menuContainer) { // Only if rendered/ref'd
                const activeIndex = allPages.findIndex((page) => isActiveRoute(page.href));
                if (activeIndex >= 0) {
                    const target = listRefs.current[activeIndex];
                    if (target) {
                        // Calculate relative to menuContainer
                        const containerRect = menuContainer.getBoundingClientRect();
                        const targetRect = target.getBoundingClientRect();

                        // We use TOP relative to the container since it flows down
                        setExpandedPillStyle({
                            transform: `translate(${targetRect.left - containerRect.left}px, ${targetRect.top - containerRect.top}px)`,
                            width: `${targetRect.width}px`,
                            height: `${targetRect.height}px`,
                            opacity: 1,
                        });
                    }
                } else {
                    setExpandedPillStyle(prev => ({ ...prev, opacity: 0 }));
                }
            }
        };

        updatePills();
        window.addEventListener("resize", updatePills);

        const intervals = [50, 150, 300];
        const timers = intervals.map(t => setTimeout(updatePills, t));

        return () => {
            window.removeEventListener("resize", updatePills);
            timers.forEach(t => clearTimeout(t));
        };
    }, [pathname, menuOpen]);

    if (!pathname.startsWith("/app")) return null;

    // Helper for styles
    const styles = {
        container: {
            "--c-glass": "#313131ff",
            "--c-light": "#ffffff49",
            "--c-dark": "#000",
            "--c-content": "#224",
            "--c-action": "#0052f5",
            "--c-bg": "#E8E8E9",
            "--glass-reflex-dark": "1",
            "--glass-reflex-light": "1",
            "--saturation": "180%",
        } as React.CSSProperties,
        dock: {
            // Replicating .switcher styles
            backgroundColor: "color-mix(in srgb, var(--c-glass) 10%, transparent)",
            backdropFilter: "blur(20px) saturate(180%)",
            WebkitBackdropFilter: "blur(20px) saturate(180%)", // Safari fallback/addition
            boxShadow: `
                inset 0 0 0 1px color-mix(in srgb, var(--c-light) calc(var(--glass-reflex-light) * 10%), transparent),
                inset 1.8px 3px 0px -2px color-mix(in srgb, var(--c-light) calc(var(--glass-reflex-light) * 90%), transparent),
                inset -2px -2px 0px -2px color-mix(in srgb, var(--c-light) calc(var(--glass-reflex-light) * 80%), transparent),
                inset -3px -8px 1px -6px color-mix(in srgb, var(--c-light) calc(var(--glass-reflex-light) * 60%), transparent),
                inset -0.3px -1px 4px 0px color-mix(in srgb, var(--c-dark) calc(var(--glass-reflex-dark) * 12%), transparent),
                inset -1.5px 2.5px 0px -2px color-mix(in srgb, var(--c-dark) calc(var(--glass-reflex-dark) * 20%), transparent),
                inset 0px 3px 4px -2px color-mix(in srgb, var(--c-dark) calc(var(--glass-reflex-dark) * 20%), transparent),
                inset 2px -6.5px 1px -4px color-mix(in srgb, var(--c-dark) calc(var(--glass-reflex-dark) * 10%), transparent),
                0px 1px 5px 0px color-mix(in srgb, var(--c-dark) calc(var(--glass-reflex-dark) * 10%), transparent),
                0px 6px 16px 0px color-mix(in srgb, var(--c-dark) calc(var(--glass-reflex-dark) * 8%), transparent)
            `,
            transition: "background-color 400ms cubic-bezier(1, 0.0, 0.4, 1), box-shadow 400ms cubic-bezier(1, 0.0, 0.4, 1), height 300ms ease-out, border-radius 300ms ease-out",
        },
        pill: {
            // Replicating .switcher::after styles
            position: "absolute" as const,
            left: 0,
            borderRadius: "99em",
            backgroundColor: "color-mix(in srgb, var(--c-glass) 36%, transparent)",
            zIndex: -1,
            boxShadow: `
                inset 0 0 0 1px color-mix(in srgb, var(--c-light) calc(var(--glass-reflex-light) * 10%), transparent),
                inset 2px 1px 0px -1px color-mix(in srgb, var(--c-light) calc(var(--glass-reflex-light) * 90%), transparent),
                inset -1.5px -1px 0px -1px color-mix(in srgb, var(--c-light) calc(var(--glass-reflex-light) * 80%), transparent),
                inset -2px -6px 1px -5px color-mix(in srgb, var(--c-light) calc(var(--glass-reflex-light) * 60%), transparent),
                inset -1px 2px 3px -1px color-mix(in srgb, var(--c-dark) calc(var(--glass-reflex-dark) * 20%), transparent),
                inset 0px -4px 1px -2px color-mix(in srgb, var(--c-dark) calc(var(--glass-reflex-dark) * 10%), transparent),
                0px 3px 6px 0px color-mix(in srgb, var(--c-dark) calc(var(--glass-reflex-dark) * 8%), transparent)
            `,
            transition: "transform 400ms cubic-bezier(1, 0.0, 0.4, 1), width 400ms cubic-bezier(1, 0.0, 0.4, 1), height 400ms cubic-bezier(1, 0.0, 0.4, 1), bottom 400ms cubic-bezier(1, 0.0, 0.4, 1), opacity 300ms ease",
            // Setting transform origin dynamically could be cool, but center is safe
            transformOrigin: "center",
        }
    };

    return (
        <>
            {/* SVG Filters - Hidden but referenced by ID */}
            <svg style={{ position: 'absolute', width: 0, height: 0, pointerEvents: 'none' }}>
                <filter id="switcher-filter" primitiveUnits="objectBoundingBox">
                    {/* The map image from the original code - encoded baseline webp 
                        Note: We are using the "toggle" map which seemed to be the main one used in .switcher 
                    */}
                    <feImage result="map" width="100%" height="100%" x="0" y="0" preserveAspectRatio="none"
                        href="data:image/webp;base64," />

                    <feGaussianBlur in="SourceGraphic" stdDeviation="0.04" result="blur" />
                    <feDisplacementMap id="disp" in="blur" in2="map" scale="0.5" xChannelSelector="R" yChannelSelector="G">
                    </feDisplacementMap>
                </filter>
            </svg>

            {/* Container */}
            <div
                className={`fixed bottom-6 left-1/2 -translate-x-1/2 z-50 max-w-md px-6 md:hidden transition-all duration-300 ${menuOpen ? "w-80" : "w-fit"}`}
                style={styles.container}
            >
                {/* Dock */}
                <div
                    ref={dockRef}
                    style={styles.dock}
                    className={`relative overflow-hidden ${menuOpen ? "h-72 rounded-3xl" : "h-16 rounded-full"} flex flex-col justify-end items-center z-40 w-full`}
                >
                    {/* Collapsed Pill */}
                    <div style={{ ...styles.pill, ...collapsedPillStyle }} />

                    {/* EXPANDED MENU CONTENT */}
                    <div
                        className={`absolute inset-x-3 bottom-[25px] transition-all duration-300 ease-out origin-bottom ${menuOpen
                            ? "opacity-100 scale-100 translate-y-0 pointer-events-auto"
                            : "opacity-0 scale-95 translate-y-4 pointer-events-none"
                            }`}
                    >
                        {/* Expanded Pill - Lives inside the container, relative to it */}
                        {/* We set top: 0 by default in style, override via transform/top if needed */}
                        <div style={{ ...styles.pill, top: 0, bottom: 'auto', transition: 'none', ...expandedPillStyle }} />

                        <div ref={menuRef} className=" p-2 pb-0">
                            <div className="flex flex-col gap-1">
                                {allPages.map((page, idx) => {
                                    const PageIcon = page.icon;
                                    const isActive = isActiveRoute(page.href);

                                    return (
                                        <Link
                                            key={page.href}
                                            href={page.href}
                                            ref={(el: HTMLAnchorElement | null) => { listRefs.current[idx] = el; }}
                                            onClick={() => setMenuOpen(false)}
                                            className={`group flex items-center gap-4 px-4 py-3 rounded-full transition-all duration-200 ${isActive
                                                ? "text-white font-medium"
                                                : "text-white/70 hover:bg-white/5"
                                                }`}
                                        >
                                            <PageIcon
                                                size={20}
                                                className={isActive ? "text-white" : "text-white/70"}
                                            />
                                            <span className="text-[15px]">
                                                {page.name}
                                            </span>
                                        </Link>
                                    );
                                })}
                            </div>
                        </div>
                    </div>

                    {/* COLLAPSED NAV ITEMS */}
                    {!menuOpen && (
                        <div className="w-full h-16 flex justify-center items-center px-1 z-50">
                            <div className="flex items-center gap-2">
                                {navItems.map((item, idx) => {
                                    const isActive = isActiveRoute(item.href);
                                    const Icon = item.icon;

                                    return (
                                        <Link
                                            key={item.href}
                                            href={item.href}
                                            ref={(el: HTMLAnchorElement | null) => { iconsRef.current[idx] = el; }}
                                            className={`relative px-5 py-3 h-full flex justify-center items-center rounded-full transition-all duration-160 ${isActive
                                                ? "text-white scale-100 cursor-default"
                                                : "text-zinc-500 dark:text-zinc-400 dark:hover:text-white hover:scale-110 cursor-pointer"
                                                }`}
                                        >
                                            <Icon size={24} strokeWidth={isActive ? 2.5 : 2} />
                                        </Link>
                                    );
                                })}
                            </div>

                            <div className="flex items-center ml-2 pl-2">
                                <button
                                    onClick={() => setMenuOpen(true)}
                                    className="p-3 px-4 flex justify-center items-center rounded-xl transition-all duration-300 text-zinc-500 dark:text-zinc-400 dark:hover:text-zinc-200"
                                >
                                    <ChevronsUpDown size={24} strokeWidth={2} />
                                </button>
                            </div>
                        </div>
                    )}
                </div>
            </div>
        </>
    );
};

export default BottomNav2;
